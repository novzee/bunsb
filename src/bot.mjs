...
// Function to send logs to admin
const sendLogToAdmin = (message) => {
...
bot.on("text", (msg) => {
    ...
    if (text === "/start") {
        ...
        sendLogToAdmin(`User ${msg.from.username || msg.from.first_name} started the bot.`);
    } else if (text === "/rules") {
        bot.sendMessage(msg.chat.id,
        ...
        sendLogToAdmin(`User ${msg.from.username || msg.from.first_name} requested the rules.`);
    }
});
...
bot.on("callback_query", (query) => {
    ...
    if (data === 'start_game') {
        ...
        sendLogToAdmin(`User ${userName} started the game.`);
        ...
        sendLogToAdmin(`User ${query.from.username || query.from.first_name} requested the rules via callback.`);
    }
});
